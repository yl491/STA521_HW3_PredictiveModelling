...
done 1. Summarize the data for 1992 noting which variables have missing data.  Which variables are categorical but are coded as numerically? 
...
done 2. Fit the logistic regression  to estimate the probability that an individual would vote Bush (Republican) as a function of income and provide a summary of the model.
```{r}
...
done 3. Obtain a point estimate and create a 95% confidence interval for the odds ratio for voting Republican for a rich person (income category 5) compared to a poor person (income category 1). *Hint this is more than a one unit change; calculate manually and then show how to modify the output from confint*. Provide a sentence interpreting the result.
```{r}
...
UNDONE!!!!!!!!ADDS ANALYSIS 5.  What does the residual deviance or any diagnostic plots suggest about the model?  (provide code for p-values and output and plots) 
```{r}
...
done 6. Create a new data set by the filtering and mutate steps above, but now include years between 1952 and 2000.
```{r}
...
done 7. Fit a separate logistic regression for each year from 1952 to 2000, using the `subset` option in `glm`,  i.e. add `subset=year==1952`.  For each find the 95% Confidence interval for the odds ratio of voting republican for rich compared to poor for each year in the data set from 1952 to 2000.
```{r}
...
#jjj<-summary(vote.glm.1952)
#jjjj<-coefficients(jjj)
#jjjj[2,1]
#oddsr<-exp(jjjj[2,1]*4)

#exp(coefficients(summary(vote.glm.1952))[2,1]*4)
...
```
done 8.  Using `ggplot` plot the confidence intervals over time similar to the display in Figure 5.4.
```{r}
...
```
\
Answer: As each party has its own distinct political ideology, so there may be interaction between them.\
...
#table(y,predict>0.5)
x<-predict(glm3,type="response")
y<-resid(glm3)
binnedplot(x,y)

binnedplot(glm3$linear.predictors,rstandard(glm3))
```
\
Answer: I don't think there is any obvious patterns for these binned points. They look random to me. Therefore, I will not recommend to add any other predictors.\
...
```
As the error rate for null is 0.412 and the error rate for our model is 0.215,our model perform better
...
kable(table)
```
According to the p value of the summary of our chosen model, four predictors has very low p value, which means we reject null and those are very significant in predicting our response variable.Those predictors are poli_id liberal, poli_id moderate,interaction of education college graduate with race of black, interaction of education high school graduate with race of balck and interaction of education some college and race of black.
...
We have identify five important coefficients.
As poli_id liberal increase by one unit, we expect the odds of voting for bush will be exp(-2.91279 ) times smaller
As poli_id moderate increase by one unit, we expect the odds of voting for bush will be  exp(-1) times smaller
As interaction of education college graduate with race of blackincrease by one unit, we expect the odds of voting for bush will be exp(-2.77)smaller
As interaction of education high school graduate with race of balck increase by one unit, we expect the odds of voting for bush will be exp(-2.78) smaller
As interaction of education some college and race of black increase by one unit, we expect the odds of voting for bush will be exp(-2.74) smaller
...
We can see that there are few missing data for partyid. It's because once people choose a party, it's very likely for the person to persist. While education changed constantly and is difficult to record, so there are more missing data for education. 
