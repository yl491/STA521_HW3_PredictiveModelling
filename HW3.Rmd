...
nes1992filter$eduction<-mapvalues(nes1992filter$educ1, from=c("1","2","3","4"), to =c("no high school","high school graduate","some college","college graduate"))
...
```{r}
lm3<-glm(formulat=vote~gender+race+eduction+partyidentification*politicalideology, data=new1992filter)
```
...
```{r}
library(arm)
expectedvalue<-predict(lm3,newdata=nes1992,type = "response")
x=expectedvalue
y=expectedvalue-nes1992$vote
binnedplot(x,y,xlab="Expected Values", ylab="Average residual", 
